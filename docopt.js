...
(function() {
  var AnyOptions, Argument, Command, Dict, DocoptExit, Either, OneOrMore, Option, Optional, Pattern, Required, TokenStream, UsageMessageError, atos, docopt, extras, formal_usage, parse_args, parse_atom, parse_doc_options, parse_expr, parse_long, parse_pattern, parse_seq, parse_shorts, print, printable_usage,
  ...
  UsageMessageError = (function(_super) {

    __extends(UsageMessageError, _super);

    function UsageMessageError(message) {
      this.message = message;
      print(this.message);
    }

    return UsageMessageError;
  ...
  TokenStream = (function(_super) {
    ...
    TokenStream.prototype.error = function(message) {
  ...
  parse_long = function(tokens, options) {
    ...
    if (opt.length > 1) {
      tokens.error("-" + raw + " is not a unique prefix");
    }
    opt = Object.create(opt[0]);
    if (opt.argcount === 1) {
      if (value === null) {
  ...
  parse_pattern = function(source, options) {
    var result, tokens;
    tokens = new TokenStream(source.replace(/([\[\]\(\)\|]|\.\.\.)/g, ' $1 '), UsageMessageError);
  ...
  printable_usage = function(doc, name) {
    return doc.split(/(usage:)/i).slice(1).join('').split(/\n\s*\n/)[0].replace(/^\s+|\s+$/, '');
  };
  ...
  docopt = function(doc, kwargs) {
    ...
    pot_arguments = (function() {
      ...
      _ref = formal_pattern.flat();
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        a = _ref[_i];
        if ((_ref1 = a.constructor) === Argument || _ref1 === Command) {
          _results.push(a);
        }
      }
      return _results;
